...
from ask_ai_dagster.defs.resources import (
    GithubResource,
    PineconeResource,
    SitemapScraperResource,
)

START_DATE = "2025-01-01"
weekly_partition = dg.WeeklyPartitionsDefinition(start_date=START_DATE)
...
def github_issues_raw(
    context: dg.AssetExecutionContext, github: GithubResource
) -> list[Document]:
    start, end = context.partition_time_window
    context.log.info(f"Finding issues between {start} and {end}")
...
def github_issues_embeddings(
    ...
    index.upsert(
        vectors=zip([str(i) for i in range(len(texts))], embeddings, metadata),
        **namespace_kwargs,
    )
...
def github_discussions_raw(
    ...
    start, end = context.partition_time_window
    context.log.info(f"Finding discussions between {start} and {end}")
...
def docs_scrape_raw(
    ...
    urls = scraper.parse_sitemap()[:10]
    documents = []

    for i, url in enumerate(urls, 1):
        document = scraper.scrape_url(url)
        if document:
            documents.append(document)
        time.sleep(0.5)
    context.add_output_metadata({"number_of_pages_scraped": len(documents)})
    return documents
